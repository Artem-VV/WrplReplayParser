...
public:
  ...
  static constexpr std::array<const char *, TYPE_COUNT + 1> ParamTypeNames = {
    "NONE",    "STRING", "INT",      "REAL",   "POINT2", "POINT3",  "POINT4", "IPOINT2",
    "IPOINT3", "BOOL",   "E3DCOLOR", "MATRIX", "INT64",  "IPOINT4", "COUNT"};
  ...
  /// Print this DataBlock (and its sub-tree) to the arbitrary text stream with limitations; returns true when whole BLK
  /// is written
...
protected:
  ...
  inline Param *getParamsImpl();

  inline const Param *getParamsImpl() const;
  ...
  inline void insertNewParamRaw(uint32_t pid, uint32_t nameId, uint32_t type, size_t type_size, const char *data);
...
namespace dblk {
  ...
  bool load(DataBlock &blk, const char *fname, ReadFlags flg = ReadFlags(), DataBlock::IFileNotify *fnotify = nullptr);
  ...
  bool print_to_text_stream_limited(const DataBlock &blk, IGenSave &cwr, int max_out_line_num = -1,
                                    int max_level_depth = -1, int init_indent = 0);
  ...
  bool export_to_json_text_stream(const DataBlock &blk, IGenSave &cwr, bool allow_unquoted = false,
                                  int max_param_per_ln = 4, int max_block_per_ln = 1);
  ...
  static inline const char *resolve_type(uint32_t type) {
    static const char *types[DataBlock::TYPE_COUNT + 1] = {"none",     "std::string", "int",     "real",    "point2",
                                                           "point3",   "point4",      "ipoint2", "ipoint3", "bool",
                                                           "e3dcolor", "tm",          "int64",   "ipoint4", "unknown"};
    type = type < DataBlock::TYPE_COUNT ? type : DataBlock::TYPE_COUNT;
    return types[type];
  }

  static inline const char *resolve_short_type(uint32_t type) {
    static const char *types[DataBlock::TYPE_COUNT + 1] = {"none", "t", "i", "r", "p2",  "p3",  "p4", "ip2",
                                                           "ip3",  "b", "c", "m", "i64", "ip4", "err"};
    type = type < DataBlock::TYPE_COUNT ? type : DataBlock::TYPE_COUNT;
    return types[type];
  }
  ...
  static inline bool is_ident_char(char c) {
    return ((c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') || c == '_' || (c >= 'A' && c <= 'Z') || c == '-' ||
            c == '.' || c == '~');
  }
  ...
  //! discard unused shared name maps (that were allocated in get_vromfs_shared_name_map() calls); return residual used
  //! count
  int discard_unused_shared_name_maps();

  //! discard unused ZSTD decoder dictionaries (that were allocated in get_vromfs_blk_ddict() calls); return residual
  //! used count
  int discard_unused_blk_ddict();

  //! returns ZSTD encoder dictionary for specified vromfs created with zstd_create_cdict (to be released with
  //! zstd_destroy_cdict)
  ...
  void pack_shared_nm_dump_to_stream(IGenSave &cwr, IGenLoad &crd, int sz, int compr_level, const ZSTD_CDict_s *cdict);
