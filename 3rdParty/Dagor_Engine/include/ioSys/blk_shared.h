...
struct DBNameMap : public DBNameMapBase {
  // void addRef() const { usageRefCount++; }
  // int delRef() const { return usageRefCount--; }
  // int getUsageRefs() const { return usageRefCount.load(); }
...
static inline To memcpy_cast(const From &f) {
  To ret;
...
template<class To>
static inline To memcpy_cast(const char *f) {
  To ret;
  memcpy(&ret, f, sizeof(To));
  return ret;
}
...
public:
  ...
  char *insertAt(uint32_t at, uint32_t n) {
    G_ASSERTF(data.size() >= at, "sz {} at {} n {}", data.size(), at, n);
...
struct DataBlockShared {
  ...
  enum { F_ROBUST_LD = 1u << 0, F_ROBUST_OPS = 1u << 1, F_BINONLY_LD = 1u << 2, F_VALID = 1u << 3 };
...
#else
  void *allocateBlock() { return malloc(sizeof(DataBlock)); }
  void deallocateBlock(void *p) { free(p); }
  void *allocateData() { return malloc(sizeof(DataBlockOwned)); }
  void deallocateData(void *p) { free(p); }
#endif

  void shrink_to_fit() { rw.shrink_to_fit(); }

  // instead of having additional indirection, we just allocate the 'rest' of DataBlockShared for data (as it is of
  // known size)
...
inline void DataBlock::insertNewParamRaw(uint32_t at, uint32_t name_id, uint32_t type, size_t type_sz, const char *nd) {
...
namespace dblk {
  enum FormatHeaderByte {
    BBF_full_binary_in_stream = '\1',
    // complete BLK with private namemap in binary stream follows to the end of the file
    BBF_full_binary_in_stream_z = '\2',
    // 3 bytes (compressed data size) and then complete BLK with private namemap in ZSTD compressed
    ...
    BBF_binary_with_shared_nm_z = '\4',
    // BLK (using shared namemap) in ZSTD compressed binary stream follows to the end of the file
    BBF_binary_with_shared_nm_zd = '\5',
    // BLK (using shared namemap) in ZSTD compressed (with dict) binary stream follows to the end of
    // the file
  };
