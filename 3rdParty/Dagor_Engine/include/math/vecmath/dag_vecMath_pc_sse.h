...
#if !defined(_TARGET_PC_LINUX) && !defined(_TARGET_PC_MACOSX) && !defined(_TARGET_PC_WIN) && !defined(_TARGET_C1) && \
  !defined(_TARGET_C2) && !defined(_TARGET_XBOX) && !defined(_TARGET_PC) && !defined(_TARGET_ANDROID)
#if defined(__EMSCRIPTEN__)
#define _TARGET_PC_EMSCRIPTEN 1
...
VECTORCALL VECMATH_FINLINE vec4f v_set_all_bits() {
  vec4f u = _mm_undefined_ps();
  return v_cmp_eq(u, u);
}
VECTORCALL VECMATH_FINLINE vec4i v_set_all_bitsi() {
  vec4i u = _mm_undefined_si128();
  return v_cmp_eqi(u, u);
}
...
NO_ASAN_INLINE vec3f v_ldu_p3_safe(const float *m) {
  return _mm_insert_ps(v_ldu_half(m), v_ldu_x(m + 2), _MM_MK_INSERTPS_NDX(0, 2, 1 << 3));
}
...
VECTORCALL VECMATH_FINLINE vec4f v_make_vec4f(float x, float y, float z, float w) { return _mm_setr_ps(x, y, z, w); }

VECTORCALL VECMATH_FINLINE vec4i v_make_vec4i(int x, int y, int z, int w) { return _mm_setr_epi32(x, y, z, w); }

VECTORCALL VECMATH_FINLINE vec4f v_make_vec3f(float x, float y, float z) { return _mm_setr_ps(x, y, z, z); }
...
VECTORCALL VECMATH_FINLINE vec4f v_splat_x(vec4f a) { return _mm_shuffle_ps(a, a, _MM_SHUFFLE(0, 0, 0, 0)); }
VECTORCALL VECMATH_FINLINE vec4f v_splat_y(vec4f a) { return _mm_shuffle_ps(a, a, _MM_SHUFFLE(1, 1, 1, 1)); }
VECTORCALL VECMATH_FINLINE vec4f v_splat_z(vec4f a) { return _mm_shuffle_ps(a, a, _MM_SHUFFLE(2, 2, 2, 2)); }
VECTORCALL VECMATH_FINLINE vec4f v_splat_w(vec4f a) { return _mm_shuffle_ps(a, a, _MM_SHUFFLE(3, 3, 3, 3)); }
...
VECTORCALL VECMATH_FINLINE bool v_test_any_bit_set(vec4f a) { return !v_test_all_bits_zeros(a); }
...
VECTORCALL VECMATH_FINLINE vec4i v_cvtu_vec4i(vec4f a) {
  ...
  return v_make_vec4i(uint32_t(_mm_cvtss_si64(a)), uint32_t(_mm_cvtss_si64(v_splat_y(a))),
                      uint32_t(_mm_cvtss_si64(v_splat_z(a))), uint32_t(_mm_cvtss_si64(v_splat_w(a))));
#else
  return v_make_vec4i(uint32_t(v_extract_x(a)), uint32_t(v_extract_y(a)), uint32_t(v_extract_z(a)),
...
VECTORCALL VECMATH_FINLINE vec4f sse4_floor(vec4f a) {
  return _mm_round_ps(a, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC);
}
VECTORCALL VECMATH_FINLINE vec4f sse4_ceil(vec4f a) {
...
v_rsqrt_est_x(vec4f a) // Reciprocal square root estimate and 1 Newton-Raphson iteration.
{
...
VECTORCALL VECMATH_FINLINE vec4f v_perm_zwcd(vec4f xyzw, vec4f abcd) {
  return _mm_shuffle_ps(xyzw, abcd, _MM_SHUFFLE(3, 2, 3, 2));
}
VECTORCALL VECMATH_FINLINE vec4f v_perm_bbyx(vec4f xyzw, vec4f abcd) {
  return _mm_shuffle_ps(abcd, xyzw, _MM_SHUFFLE(0, 1, 1, 1));
}
VECTORCALL VECMATH_FINLINE vec4f v_perm_xaxa(vec4f xyzw, vec4f abcd) {
  return v_perm_yzxw(_mm_shuffle_ps(xyzw, abcd, _MM_SHUFFLE(0, 0, 0, 0)));
}
VECTORCALL VECMATH_FINLINE vec4f v_perm_yybb(vec4f xyzw, vec4f abcd) {
  return _mm_shuffle_ps(xyzw, abcd, _MM_SHUFFLE(1, 1, 1, 1));
}
...
VECTORCALL VECMATH_FINLINE vec4f sse2_dot3(vec4f a, vec4f b) {
  ...
  return _mm_add_ps(_mm_add_ps(_mm_shuffle_ps(m, m, _MM_SHUFFLE(2, 2, 1, 0)), V_SHUFFLE_REV(m, 1, 1, 0, 2)),
                    _mm_shuffle_ps(m, m, _MM_SHUFFLE(0, 0, 2, 1)));
}
VECTORCALL VECMATH_FINLINE vec4f sse2_dot4(vec4f a, vec4f b) {
  vec4f m = _mm_mul_ps(a, b);

  return _mm_add_ps(_mm_add_ps(m, V_SHUFFLE_REV(m, 2, 1, 0, 3)),
                    _mm_add_ps(_mm_shuffle_ps(m, m, _MM_SHUFFLE(1, 0, 3, 2)), V_SHUFFLE_REV(m, 0, 3, 2, 1)));
}
...
VECTORCALL VECMATH_FINLINE vec4f sse2_dot4_x(vec4f a, vec4f b) {
  ...
  return _mm_add_ss(_mm_add_ss(m, v_splat_y(m)), _mm_add_ss(_mm_shuffle_ps(m, m, 2), v_splat_w(m)));
}

VECTORCALL VECMATH_FINLINE vec4f sse2_plane_dist_x(plane3f a, vec3f b) {
  return v_add_x(sse2_dot3_x(a, b), v_splat_w(a));
}
...
VECTORCALL VECMATH_FINLINE vec4f sse4_plane_dist_x(plane3f a, vec3f b) {
  return v_add_x(sse4_dot3_x(a, b), v_splat_w(a));
}
#else // fallback to SSE2
...
VECTORCALL VECMATH_FINLINE vec4f sse3_plane_dist_x(plane3f a, vec3f b) {
  return v_add_x(sse2_dot3_x(a, b), v_splat_w(a));
}
#else
...
VECTORCALL VECMATH_FINLINE void v_mat_43cu_from_mat44(float *__restrict m43, const mat44f &tm) {
  ...
  vec4f v0 = _mm_insert_ps(tm.col0, tm.col1, _MM_MK_INSERTPS_NDX(0, 3, 0));
  vec4f v1 = v_perm_xyab(v_rot_1(tm.col1), tm.col2);
  vec4f v2 = _mm_insert_ps(_mm_castsi128_ps(_mm_slli_si128(_mm_castps_si128(tm.col3), 4)), tm.col2,
                           _MM_MK_INSERTPS_NDX(2, 0, 0));
#else // _TARGET_SIMD_SSE >= 4
  vec4f v0 = v_perm_xyzd(tm.col0, v_splat_x(tm.col1));
  vec4f v1 = v_perm_xyab(v_rot_1(tm.col1), tm.col2);
...
VECTORCALL VECMATH_FINLINE vec4f v_mat44_mul_vec3v(mat44f_cref m, vec3f v) {
  ...
  return _mm_add_ps(_mm_add_ps(_mm_mul_ps(xxxx, m.col0), _mm_mul_ps(yyyy, m.col1)), _mm_mul_ps(zzzz, m.col2));
}
...
#else
  int64_t t;
  _mm_storel_epi64((__m128i *) &t, v_roti_2(a));
  return t;
#endif
}
#endif
